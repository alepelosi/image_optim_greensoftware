...
This branch enables the energy-oriented green profile by default so the benchmark script can measure it without changing its command line. Green mode applies lower-cost worker defaults, caps default parallelism to two threads, uses a 30 second per-image timeout, and stops launching remaining workers once the current output is at least 50% smaller than the original. Disable it with:
...
The green threshold can be tuned with `green_threshold` or `--green-threshold`.
